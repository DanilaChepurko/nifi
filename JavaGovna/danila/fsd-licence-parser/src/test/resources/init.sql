CREATE TABLE IF NOT EXISTS public.business_associate (
    uuid UUID NOT NULL PRIMARY KEY,
    long_name VARCHAR(255) CONSTRAINT idx_unique_business_associate_long_name UNIQUE,
    short_name VARCHAR(255),
    ba_type VARCHAR(255),
    stockholders VARCHAR(1000)
);

-- расположение учатска недр
CREATE TABLE IF NOT EXISTS subsoil_area (uuid UUID PRIMARY KEY, name VARCHAR(255) NOT NULL);

CREATE TABLE horizon_document (
    uuid UUID PRIMARY KEY,
    ba_uuid UUID NOT NULL,
    horizon_uuid UUID NOT NULL,
    field_uuid UUID NOT NULL,
    pool_uuid UUID NOT NULL,
    -- год выдачи лицензии
    licence_year INTEGER NOT NULL,
    licence_code VARCHAR(255) NOT NULL
);

CREATE TABLE IF NOT EXISTS horizon_document_dop_info (
    horizon_document_uuid UUID primary key NOT NULL,
    -- Год начала промышленной разработки залежи
    dev_start_year INTEGER NOT NULL,
    -- Год превышения степени выработанности залежи 1% (только для туронской залежи)
    turon_1pct_year INTEGER,
    oil_license_year INTEGER,
    oil_geo_license_year INTEGER,
    -- Год, когда степень выработки впервые превысила 1% (нефть)
    oil_1pct_year INTEGER,
    -- Степень выработки по лицензионному участку на 01.01.2011 (только для нефти)
    oil_recovery_2011 NUMERIC(6, 4),
    -- Предел добычи по льготе Ккан, млн т (нефть)
    oil_kkan_limit NUMERIC(10, 4),
    -- Последний год применения льготы Ккан (нефть)
    oil_kkan_last_year INTEGER,
    -- Участок недр для льготы по НДПИ (нефть)
    subsoil_area_uuid UUID
);

create table if not exists public.field (
    uuid uuid not null constraint field_pk primary key,
    name varchar(255),
    ba_uuid uuid,
    area_uuid uuid,
    type_uuid uuid,
    licenses varchar(255),
    esg boolean,
    int integer
);

create table if not exists public.horizon (
    uuid uuid not null constraint horizon_pk primary key,
    name varchar(255) constraint idx_unique_horizon_name unique,
    field_uuid uuid
);

create table if not exists public.pool (
    uuid uuid not null constraint pool_pk primary key,
    name varchar(255),
    ba_uuid uuid,
    field_uuid uuid,
    horizon_uuid uuid,
    esg boolean,
    license boolean,
    drod integer,
    licenses varchar(500)
);

create table if not exists public.area (
    uuid uuid not null constraint area_pk primary key,
    name varchar(255)
);

INSERT INTO
    public.pool (
        uuid,
        name,
        ba_uuid,
        field_uuid,
        horizon_uuid,
        esg,
        license,
        drod,
        licenses
    )
VALUES
    (
        'd0367fcb-3a33-406a-b35d-3efb936bccd1',
        'Медвежье.Неоком.I',
        null,
        '3b30f79e-2a27-4af1-a5a0-eca388f220a0',
        'a11a99a7-1ed3-4b07-ab4d-863e8f71ba0d',
        null,
        null,
        null,
        null
    ),
    (
        '3e46a3ef-845b-43e0-b5cb-2f0a0d8e13d7',
        'Медвежье.Ачимовский.Iн',
        null,
        '3b30f79e-2a27-4af1-a5a0-eca388f220a0',
        'a11a99a7-1ed3-4b07-ab4d-863e8f71ba0d',
        null,
        null,
        null,
        null
    );

INSERT INTO
    public.field (
        uuid,
        name,
        ba_uuid,
        area_uuid,
        type_uuid,
        licenses,
        esg,
        int
    )
VALUES
    (
        '3986ea1c-ac29-4cf7-8a82-5e12a1063b80',
        'Медвежье',
        'f271eb01-0b47-45dd-843b-d05ee20dbe8c',
        '1a0b5abb-473d-42d3-8434-43ab5862da30',
        '20c668bf-e0de-4e41-897e-513db70be197',
        'ШОМ006641НР 19.08.2022',
        false,
        null
    );

INSERT INTO
    public.business_associate (uuid, long_name, short_name, ba_type)
VALUES
    (
        '99f91cef-ea38-4aa2-8956-98762c406d9a',
        'ООО "Газпром добыча Надым"',
        'ООО "Газпром добыча Надым"',
        null
    );

INSERT INTO
    public.horizon (uuid, name, field_uuid)
VALUES
    (
        'cc2ca3c1-eeeb-4767-8bc6-28ed5c874330',
        'Медвежье.Ачимовский',
        '3986ea1c-ac29-4cf7-8a82-5e12a1063b80'
    ),
    (
        'c040e431-70db-401c-9e27-f36eb40fee4f',
        'Медвежье.Неоком',
        '3986ea1c-ac29-4cf7-8a82-5e12a1063b80'
    ),
    (
        '3ce66fd8-60f5-44c6-84ac-46e7642d6f37',
        'Медвежье.Сеноман',
        '3986ea1c-ac29-4cf7-8a82-5e12a1063b80'
    );

INSERT INTO
    public.subsoil_area (uuid, name)
VALUES
    (
        '63bc3f5f-053a-45b6-87b2-3cfa670adbfa',
        'иное расположение'
    );